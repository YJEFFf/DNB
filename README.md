...
매일 아침 8시(KST), 네이버 뉴스에서 5대 종합일간지(조선일보/중앙일보/동아일보/한겨레/경향신문)의 최근 24시간 기사를 카테고리 구분 없이 수집하고, Claude가 오늘의 주요 뉴스를 직접 선정·요약해 텔레그램으로 보내주는 자동화 프로젝트입니다.
...
GitHub Actions가 매일 UTC 23:00(= KST 08:00)에 자동 실행합니다.
...
`.github/workflows/daily-news.yml`의 `cron` 표현식을 수정하세요. 예를 들어 KST 오전 7시로 바꾸려면 `cron: "0 22 * * *"` (UTC 22:00)로 변경합니다.

> ⚠️ GitHub Actions의 스케줄 cron은 무료 러너 부하에 따라 실제 실행이 몇 분~몇십 분 지연될 수 있습니다. 정확한 시간이 중요하다면 별도 서버의 cron으로 옮기는 것을 고려하세요.
